...
export const JobModal: React.FC<JobModalProps> = ({ evaluation, onClose, onEvaluate, isEvaluating }) => {
  ...
  // Check for existing tailored versions on open
  useEffect(() => {
    if (evaluation?.job_id) {
      apiClient.getTailoredVersions(evaluation.job_id).then(versions => {
        if (versions && versions.length > 0) {
          // If there's an approved one, load it? Or just the latest?
          // Let's just store the latest for now if user wants to see it
          // Actually maybe we don't auto-open it, just knowing it exists is enough?
          // For now, let's just leave it null until user clicks 'Tailor' (which creates new) or we add a 'View Tailored' button.
          // Re-think: If we already have one, showing "Tailor" again creates a NEW version. That's fine.
        }
      });
    }
  }, [evaluation?.job_id]);
  ...
  const handleTailor = async () => {
    setIsTailoring(true);
    try {
      // 1. Fetch Base Resume (if not already)
      let base = baseResume;
      if (!base) {
      ...
      // 2. Trigger Tailoring
      const result = await apiClient.tailorResume(evaluation.job_id);
            ...
            <div className="space-y-6">
              ...
              {evaluation.recommended_action === 'tailor' && (
                <button
                  ...
                  className="w-full py-4 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white rounded-xl font-bold text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all flex items-center justify-center disabled:opacity-70 disabled:hover:scale-100"
                >
                  {isTailoring ? (
                    <>
