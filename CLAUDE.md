...
- `docs/active/ootocv-frontend-wiring.md` — OotoCV frontend rebuild matching `references/OotoCV.zip` 🟡 (TypeScript clean; awaiting browser smoke)
...
Recorded in `docs/decisions/`. ADR-0001 through ADR-0026 cover all major architectural shifts.
...
- ADR-0026: Adopt OotoCV reference shell (single-column fat-card feed, dedicated `/job/:id` and `/roast` routes, 5-step Onboarding, reference Sidebar with rotating cron messages). Supersedes ADR-0007's two-pane Linear layout. `services/jobAdapter.ts` collapses the backend's `Job` + `Evaluation` + `ParseResult` into the reference's flat `ReferenceJob` shape. Per-change accept/reject UI, pipeline mode pills, and run grouping are not surfaced in the reference shell; backend support remains.
